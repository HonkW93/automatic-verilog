"=============================================================================
" Vim Plugin for Verilog Code Automactic Generation (RtlTree)
" Verilog自动化插件（带RTL树）
" 
" Language:       Verilog
" Maintainer:     zhangguo,Modified by HonkW
" Version:        2.2.4
...
"Variable & Config & Type 变量/配置/类型定义{{{1


"Variable Define 定义变量值及边界 {{{2
...
"Menu definition 菜单栏定义{{{1

"Wave波形
amenu &Wave.AddClk                                                      :call AddClk()<CR>
amenu &Wave.AddSig                                                      :call AddSig()<CR>
amenu &Wave.AddBus                                                      :call AddBus()<CR>
amenu &Wave.AddBlk                                                      :call AddBlk()<CR>
amenu &Wave.-Operation-                                                 :
amenu &Wave.Invert<TAB><C-F8>                                           :call Invert()<CR>

"Code代码段
amenu &Code.Always@.always\ @(posedge\ or\ posedge)<TAB><al>            :call AlBpp()<CR>i
amenu &Code.Always@.always\ @(posedge\ or\ negedge)                     :call AlBpn()<CR>
amenu &Code.Always@.always\ @(*)                                        :call AlB()<CR>
amenu &Code.Always@.always\ @(negedge\ or\ negedge)                     :call AlBnn()<CR>
amenu &Code.Always@.always\ @(posedge)                                  :call AlBp()<CR>
amenu &Code.Always@.always\ @(negedge)                                  :call AlBn()<CR>

amenu &Code.-header-                                                    : 
amenu &Code.AddHeader<TAB><header>                                      :call AddHeader()<CR>

amenu &Code.-comment-                                                   : 
amenu &Code.SingleLineComment<TAB><F2>                                  :call AutoComment()<CR>
amenu &Code.MultiLineComment<TAB><F3>                                   :call AutoComment2()<CR>
amenu &Code.CurLineAddComment<TAB><F4>                                  :call AddCurLineComment()<CR>

"连线
amenu &Verilog.-Misc-                                                   :
amenu &Verilog.WireDef2PortConn<TAB><NA>                                :call WireDef2PortConn()<CR>
amenu &Verilog.Input<->Output<TAB><NA>                                  :call Input2Output()<CR>
...
amenu &Verilog.AutoInst\ -\ all<TAB><S-F4>                              :call AutoInst(1)<CR>
...
"Command definition 快捷键定义{{{1

"Emacs-Verilog-mode{{{2

"添加emacs之后可使用部分emacs的功能，快捷键调用即可，与本插件的功能基本一致,有一点比较好的是可以跨目录,其余功能相差不大
...
"选中单词声明为input
:map ;i ebmsd^:.s/^ *\([a-zA-Z][a-zA-Z0-9_]*\).*$/    input   \1 ,/<cr><F7>`sj:noh<cr>

"选中单词声明为output wire
:map ;ow ebmsd^:.s/^ *\([a-zA-Z][a-zA-Z0-9_]*\).*$/    output  wire    \1 ,/<cr><F7>`sj:noh<cr>

"选中单词声明为output reg
:map ;or ebmsd^:.s/^ *\([a-zA-Z][a-zA-Z0-9_]*\).*$/    output  reg     \1 ,/<cr><F7>`sj:noh<cr>
...
:map <F7> 0f,bi                                                   <ESC>^32ldwf,i                                                   <ESC>^64ldwa        <ESC>dw^

"调整reg，wire所在行的变量，逗号和注释的位置
:map <F8> 0f;bi                                                   <ESC>^32ldwf;i                                                   <ESC>^64ldwa        <ESC>dw^
...
"Search instance / module definition 搜索instance或module{{{1
...
"AutoTemplate definition 快速新建.v文件/模板更新{{{1
...
"Always Block definition 快速添加always块{{{1
...
"AutoComment definition 快速添加注释"{{{1
...
"Line Number definition 统计行字数"{{{1
